...
export function switchTab(targetTab, triggerPushState = true) {
  ...
  const playingVidId = window.currentPlayingVideoId || null;
  const activeVideoInstance = window.videoPlayerInstance || null;
  ...
  if (activeTab === 'downloaded' && targetTab !== 'downloaded') {
    ...
    if (isInlineOpen && playingVidId) {
      const video = (localDb.history || []).find(h => h.id === playingVidId);
      ...
      if (modal && modalBody && inlineBody && inlineBody.firstElementChild) {
        ...
        if (typeof window.resetAndApplyPlayerDimensions === 'function') {
          window.resetAndApplyPlayerDimensions(isShort, true); // Minimized modunda aç
        }
        ...
        if (activeVideoInstance && typeof activeVideoInstance.resize === 'function') {
          setTimeout(() => activeVideoInstance.resize(), 50);
        }
...
export function getCurrentPlaybackState() {
  ...
  if (pType === 'artplayer' && activeVideoInstance) {
    currentTime = activeVideoInstance.currentTime || 0;
    paused = activeVideoInstance.paused;
  } else if (pType === 'html5' && player) {
    currentTime = player.currentTime || 0;
    paused = player.paused;
  } else if (activeVideoInstance) {
    currentTime = activeVideoInstance.currentTime || 0;
    paused = activeVideoInstance.paused;
  } else if (player) {
    currentTime = player.currentTime || 0;
...
export function saveDownloadedFilterState() {
  try {
    ...
    const state = {
      channel: channelSelect ? channelSelect.value : (window.downloadedFilterChannel || 'all'),
      sortVal: currentSort,
      showShorts: showShortsCb ? showShortsCb.checked : false,
      viewMode: window.downloadedViewMode || 'grid'
    };
...
export function initTabManager(getState) {
  ...
  document.addEventListener('change', (e) => {
    if (e.target && e.target.id === 'history-channel-filter') {
      onHistoryChannelFilterChange();
    } else if (e.target && e.target.id === 'history-date-filter') {
      onHistoryDateFilterChange();
    } else if (e.target && e.target.id === 'history-duration-filter') {
      onHistoryDurationFilterChange();
    } else if (e.target && e.target.id === 'downloaded-channel-filter') {
      onDownloadedChannelFilterChange();
    }
  });
}

// Türkçe Açıklama: Kütüphane tarih filtre dropdown seçimi değiştiğinde state kaydeder ve UI günceller.
export function onHistoryDateFilterChange() {
  const select = document.getElementById('history-date-filter');
  if (select) {
    window.historyFilterDays = select.value;
    saveHistoryFilterState();
    if (typeof window.updateUI === 'function') window.updateUI(localDb);
  }
}

// Türkçe Açıklama: Kütüphane süre filtre dropdown seçimi değiştiğinde ayarları kaydeder ve UI günceller.
export function onHistoryDurationFilterChange() {
  const select = document.getElementById('history-duration-filter');
  if (select) {
    if (!localDb.settings) localDb.settings = {};
    localDb.settings.historyDurationFilter = select.value;
    saveHistoryFilterState();
    try {
      fetch('/api/settings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...localDb.settings, historyDurationFilter: select.value })
      }).catch(() => {});
    } catch (e) {}
    if (typeof window.updateUI === 'function') window.updateUI(localDb);
  }
...
// Türkçe Açıklama: Kütüphane kanal filtre dropdown seçimi değiştiğinde filtre uygular ve UI günceller.
export function onHistoryChannelFilterChange() {
  const select = document.getElementById('history-channel-filter');
  if (select) {
    window.historyFilterChannel = select.value;
    saveHistoryFilterState();
    if (typeof window.updateUI === 'function') window.updateUI(localDb);
  }
}

// Türkçe Açıklama: İndirilenler kanal filtre dropdown seçimi değiştiğinde filtre uygular ve UI günceller.
export function onDownloadedChannelFilterChange() {
  const select = document.getElementById('downloaded-channel-filter');
  if (select) {
    window.downloadedFilterChannel = select.value;
    saveDownloadedFilterState();
    if (typeof window.updateUI === 'function') window.updateUI(localDb);
  }
}
...
if (typeof window !== 'undefined') {
  ...
  window.onDownloadedChannelFilterChange = onDownloadedChannelFilterChange;
