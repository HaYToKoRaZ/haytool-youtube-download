...
- **UI & UX (Özelleştirilmiş Silme Modalı & Kütüphanede Gizleme Seçeneği):** Yerleşik video oynatıcı altındaki "Sil" butonuna tıklandığında standart tarayıcı uyarısı (`confirm()`) yerine zengin seçenekler içeren özelleştirilmiş `#delete-modal` penceresinin açılması sağlandı. Silme butonuna basıldığında video otomatik duraklatılır ve kullanıcıya 3 bağımsız seçenek sunulur:
  ...
  2. "YouTube'da da izlendi olarak işaretle" (`#mark-watched-checkbox`)
  3. "Videoyu kütüphanede gizlenenlere ekle" (`#hide-library-checkbox`)
- **Fix & Player Lifecycle (Silme Onayında Oynatıcıyı Anında Kapatma):** Oynatıcı açıkken modal üzerinden silme onaylandığında oynatıcının ekranda asılı kalması ve ancak "Kapat" butonuna basınca listenin görünmesi sorunu çözüldü. `#downloaded-inline-player-container` ve `#player-modal` elementleri silme onaylandığı anda derhal gizlenir, `downloaded-list-container` açılır, tüm medya akışları ve soketler temizlenir, dosya kilidi bırakılması için 350ms emniyet payı tanınır ve liste temizlenir.
- **Fix & Engine (Silme Tercihleri Senkronizasyonu & Backend Desteği):** Silme modalındaki "Kütüphanede de gizle" seçeneği `settings.hideOnDelete` ve `localStorage` ile senkronize edildi. `server/routes/history.js` içerisindeki tekil (`DELETE /api/history/:id`) ve toplu (`POST /api/history/bulk-delete`) silme uç noktalarına `hideOnDelete` parametre desteği eklendi. Silme işlemi onaylandığında açık olan oynatıcılar (`closeInlinePlayer(true)`, `closePlayerModal(true)`) dosya kilidi bırakmayacak şekilde derhal kapatılır, 350ms emniyet süresi ardından silme gerçekleştirilir.
- **Fix & Resilience (Akış Soketi İletişim Hatası & Windows Dosya Kilidi Retry):** Video izlenirken oynatıcı kapatıldığında WebView2 medya akış soketinin kopması nedeniyle ortaya çıkan sahte "Sunucu ile iletişim hatası." toast uyarısı giderildi; silme işlemi başarılı olduğunda sistem veritabanı durumunu teyit ederek başarı toast'ı gösterir. `server/routes/history.js` içerisine Windows NTFS dosya kilitlerine (`EBUSY` / `EPERM`) karşı 4 denemeli (150ms aralıklı) akıllı dosya silme retry döngüsü eklendi.
- **i18n & Documentation (7 Dil Desteği & mapsui.md Güncellemesi):** `#label-hide-library-modal` etiketi için 7 dilde (`tr`, `en`, `de`, `es`, `pt`, `ru`, `ar`) yerelleştirme tanımlandı. `0nogithub/mapsui.md` bileşen haritası yeni kontrol kutusu ile güncellendi.

## [9.8.23] - 2026-09-12
- **Refactor & Modularization (IPTV Modülü & app.js Hafifletme - A Seçeneği):** Kural 1 (Atomic Design) gereğince, `public/app.js` içerisindeki tüm IPTV kanal listesi, sayfalama, filtreler, 1-2-4 slotlu M3U8 oynatıcı kontrolleri ve spor modu sürükle/boyutlandır mantığı (1.192 satır) `public/modules/iptv.js` modülüne taşındı. `iptv.js` içerisine `getLocalDb()` emniyet köprüsü ve `window` API entegrasyonu sağlandı. `public/app.js` boyutu **449.98 KB'tan 409.52 KB'a (9.677 satır)** düşürüldü.
- **Refactor & Architecture (Codebase Memory Entegrasyonu & app.js Hafifletme):** `public/app.js` içerisindeki 693 satırlık devasa `applyLanguage` fonksiyonu Kural 1 (Atomic Design) standardına göre `public/modules/i18n-apply.js` modülüne taşındı. `app.js` dosya boyutu 547.8 KB'tan 449.98 KB'a (~98 KB) düşürülerek istemci performansı optimize edildi.
- **Performance & Backend (Bellek İçi Önbellek & Ölü Kod Temizliği):** `server/database.js` içerisine 1500ms TTL bellek içi önbellek eklendi; yüksek frekanslı `readDb()` disk I/O yükü minimize edildi. Projede referansı bulunmayan `class DatabaseORM` (84 satır) ve `server/services/rss.js` içerisindeki eski `checkNextChannelRss` fonksiyonu (44 satır) temizlendi.
- **Refactor (Birleşik YouTube Çerez Başlığı):** `server/services/cookieHealth.js` içindeki `buildCookieHeader()` fonksiyonu dışa aktarılabilir hale getirildi ve `server/routes/history.js` içerisindeki mükerrer çerez ayrıştırıcı silinerek tek çatı altında toplandı.
- **Refactor (C# Tray INI Okuma Birleştirmesi):** `tray.cs` içerisindeki `GetDiscordRpcSetting()` ve `GetUseAlternativeSpeedSetting()` metotları birleştirilerek tek bir `GetIniBoolSetting` yardımcı metoduna dönüştürüldü.
- **Fix & Engine (yt-dlp Nightly Motor Kalıcılığı):** Ayarlar sekmesindeki sürüm denetim tutarsızlığı giderildi. Yerel Python pip ortamı ve `yt-dlp/yt-dlp.exe` ikili motoru en güncel Nightly `2026.08.30.232658` sürümü ile tam eşitlendi.
- **Documentation & CBM:** `0nogithub/maps.md` ve `0nogithub/clinerules.md` proje haritası, `D:\codebase-memory\sistem\D-Program-Files-HaYTooL-YT-Downloader.db` SQLite AST ve semantik bilgi grafı ile senkronize edildi.

## [9.8.22] - 2026-09-12
- **Feature (İndirilenler Sekmesi - Yarım Kalan Videolar Filtresi):** İndirilen videolar sekmesine (`#tab-downloaded`) izlenmesi yarıda bırakılan videoları tek tıkla izole eden "Yarım Kalanlar" (`#downloaded-filter-resume-btn`) filtre butonu eklendi.
- **UI & UX (Dinamik Rozet Sayacı & Mor Cam Efekti):** Yarım bırakılan video adedini canlı gösteren dinamik mor rozet sayacı (`#badge-resume-count`) eklendi. Filtre açıkken mor cam/ışıma efekti uygulanır. Filtre durumu localStorage üzerinde kalıcı saklanır (`onlyResume`).
- **i18n (7 Dil Desteği):** Yeni filtre için 7 dilde (`tr`, `en`, `es`, `de`, `pt`, `ru`, `ar`) buton metinleri ve ipucu etiketleri tanımlandı.
- **Single Instance & Sessiz YouTube Giriş İyileştirmeleri:** Bildirim tepsisi sağ tık menüsünden oynatıcı tetiklendiğinde çift kopya oluşumu engellendi, açılıştaki YouTube çerez penceresi arka planda ve 30 saniye zaman aşımı ile çalışacak biçimde optimize edildi.

## [9.8.21] - 2026-09-07
- **Feature (Kanallar Sekmesi Kategori Filtresi):** Takip edilen kanallar sekmesine (`#tab-channels`) arama ve indirme filtrelerinin yanına dinamik "Kategori" filtresi dropdown'u eklendi. Kullanıcılar atanmış kategorilere göre kanalları seri ve anlık olarak filtreleyebilir, belirli bir kategoriye sahip veya kategorisiz kanalları izole edebilir.
- **UI & UX (Dinamik Kategori Sayaçları & Akıllı Filtreleme):** Kategori filtresi seçeneklerinde her kategoriye ait aktif kanal sayıları `Oyun (5)`, `Genel (12)` şeklinde gösterilir. A-Z alfabetik gruplama ve üstteki filtrelenen kanal sayaç rozeti (`filtered / total`) anlık senkronize edilir.
- **i18n (7 Dil Desteği):** Kategori filtre başlıkları, "Tüm Kategoriler" ve "Atanmamış / Kategorisiz" seçenekleri 7 dilde (`tr`, `en`, `es`, `de`, `pt`, `ar`, `ru`) tam olarak lokalize edildi ve dinamik dil değişimine bağlandı.

## [9.8.20] - 2026-09-07
- **Bugfix (HaYTooL-Player Beta Başlatıcı & Port Koruması):** `0nogithub/player_launcher.cs` içerisindeki süreç kontrolü güncellendi; arka planda asılı kalan görünür penceresi olmayan (`MainWindowHandle == IntPtr.Zero`) zombi süreçlerin kullanıcının oynatıcıyı açmasını engellemesi (`return;` ile sessizce kapanması) sorunu çözüldü. Sadece aktif ve görünür oynatıcı pencereleri öne getirilir, aksi halde yeni oynatıcı penceresi açılır. Ayrıca port TCP üzerinden kontrol edilerek sunucu zaten çalışırken `HaYTooL YT Downloader.exe`'nin tekrar tetiklenmesi ve port çakışması uyarısı vermesi engellendi.
- **Bugfix (WebView2 Bağlantı Reddi ve Yarış Durumu Koruması):** `HaYTooLPlayer/MainWindow.xaml.cs` içerisine `WaitForBackendReadyAsync` yoklama mekanizması eklendi. Oynatıcı açıldığında backend sunucusu (`http://127.0.0.1:4141/api/version`) hazır olana kadar beklenir; böylece sunucu henüz ayağa kalkarken WebView2'nin `ERR_CONNECTION_REFUSED` alarak boş beyaz sayfada kalması önlendi.
- **Bugfix (Süreç Kapanma ve UI Kilitlenme Giderimi):** `MainWindow.xaml.cs` ve `App.xaml` içerisinde `ShutdownMode="OnMainWindowClose"` tanımlandı; `SyncYouTubeCookiesToFileAsync()` işlemi UI thread'ini dondurmayacak şekilde asenkron arka plan görevine devredildi. Sessiz çerez yenileme (`HaYTooLPlayer_Silent`) ile ana oynatıcı (`HaYTooLPlayer_Main`) profil klasörleri birbirinden ayrılarak WebView2 kilitlenme riski ortadan kaldırıldı.
- **Stability (Sessiz Çerez Yenileme Emniyet Zaman Aşımı):** `--silent-cookie-refresh` modunda çalışan gizli `HaYTooLPlayer.exe` süreçleri için 15 saniyelik zorunlu kapanma sayacı (`DispatcherTimer`) eklendi. Olası internet veya sayfa takılmalarında sürecin bellekte süresiz asılı kalması engellendi.
- **Stability (Backend Monitörü & Çoklu INI Desteği):** `StartBackendMonitor` agresif sayaç kapanması yerine soket dinleme mantığına geçirildi; port üzerinden backend yanıt verirken oynatıcının yanlışlıkla kapanması engellendi. `GetAppUrl()` fonksiyonu hem `bin/` hem ana klasördeki `configwin.ini` dosyalarını tanıyacak şekilde güncellendi.

## [9.8.19] - 2026-09-07
- **Feature (Kuyruk Sekmesi 2 Sütunlu Geçmiş Alanı):** Kuyruk sekmesinin alt bölümü iki sütunlu ızgara yapısına kavuşturuldu: Sol sütunda "Son İndirilen Videolar" (`#queue-completed-list`), sağ sütunda ise "Hata Alanlar & Denenenler" (`#queue-failed-list`) listelenir. Hem Tablo hem de Kart görünüm modları ile tam uyumlu çalışır.
- **Feature (Hata Alanları Yönetme & Yeniden İndirme):** Hata alan indirmeler için tekil yeniden deneme (`retryFailedVideo`), toplu tümünü tekrar deneme (`retryAllFailedVideos`), tekil temizleme (`clearFailedVideo`) ve toplu temizleme (`clearAllFailedVideos`) butonları ve aksiyonları eklendi.
- **Feature (Kuyruk Sıralama Hızlı Taşıma Butonları):** Sıradaki videolar listesinde tek tek tıklama ihtiyacını ortadan kaldırmak için yukarı/aşağı oklarının yanına "En Başa Taşı" (`chevrons-up`) ve "En Sona Taşı" (`chevrons-down`) butonları eklendi.
- **i18n (7 Dil Desteği):** Yeni eklenen tüm arayüz başlıkları, hata rozetleri ve buton etiketleri için 7 dilde (`tr`, `en`, `es`, `de`, `pt`, `ar`, `ru`) dil anahtarları tanımlandı.
